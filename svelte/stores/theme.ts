...
export interface ThemeStore {
	subscribe: (fn: (value: ThemeStoreValue) => void) => () => void;
...
export function createThemeStore(config: ThemeConfig): ThemeStore {
	...
	function loadFromLocalStorage(): ThemeSettings {
		...
		try {
			const cookieSettings: Partial<ThemeSettings> = readPreferenceCookies();
	...
	const subscribers = new Set<(value: ThemeStoreValue) => void>();

	const subscribe = (fn: (value: ThemeStoreValue) => void): (() => void) => {
		subscribers.add(fn);
		fn({
			settings,
			theme,
